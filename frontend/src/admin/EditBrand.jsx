...
export default function EditBrand() {
  ...
  useEffect(() => {
    const getBrandDetails = async () => {
      try {
        ...
        const res = await axiosinstance.get(`/brand/${brand_id}`);
        
        if (res.data.success) {
          
          const Branddata = res.data.allBrands; 
          ...
          if (Branddata) {
            ...
            if (Branddata.image) {
              ...
              const cleanImagePath = Branddata.image.startsWith('/') ? Branddata.image.slice(1) : Branddata.image;
  ...
  try {
    ...
    const res = await axiosinstance.patch(`/brand/editbrand/${brand_id}`, payload, {
    ...
    if (res.data.success) {
      notify(res?.data?.message || "Brand updated!", true);
      clearform();
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <div className="text-sm font-black text-gray-400 tracking-widest uppercase animate-pulse">
          Loading original Brands data node...
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-lg bg-white shadow-md rounded-2xl p-6 border border-gray-100">
        ...
        <form onSubmit={submithandler} className="space-y-5 text-xs font-bold tracking-tight text-gray-700">
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {loading ? "Syncing Modifications..." : "Save Brand Modifications"}
            </button>
          </div>
        </form>
      </div>
