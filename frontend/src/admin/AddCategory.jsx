...
export default function AddCategory() {
  ...
  function SubmmitHandler(event) {
    ...
    const categorySlug = slugRef.current.value.trim();
    const categoryStatus = statusRef.current.checked;
    const categoryImage = imageInputRef.current.files[0];
    ...
    formData.append("name", categoryName); // Unified layout aesthetics
    formData.append("slug", categorySlug);
    formData.append("status", categoryStatus); // 🔥 Missing status payload injected
